...
export const searchComponentsInput = z.object({
  tags: z
    ...
    .optional()
    .describe(
      'Filter by tags (array overlap). Prefer over freeText when a canonical tag matches intent.',
    ),
  category: z.string().optional(),
  inputFormat: z.string().optional(),
  freeText: z.string().optional().describe('Fallback only. Use when no tag fits. 1–2 keywords.'),
});
```
...
A single SQL statement with `ILIKE OR FTS OR trigram` conditions is fine (deterministic for one input). Sequential retries with different inputs are the problem. For autocomplete, fuzzy-match UX, or "show _something_ even if imperfect" scenarios, silent fallbacks are OK — they improve perceived responsiveness at the cost of strict stability.
