...
2. For each chunk, prompt a cheap model (GPT-4o-mini, Claude Haiku):
   _"Generate a natural \[language\] question that this text answers."_
...
2. Rerankers and query rewriters look _worse_ than they will in production —
...
const hitAt = (k: number) => results.filter(r => r.rank > 0 && r.rank <= k).length / results.length;
