...
const ActivityCard: FC<Props> = ({
  ...
  return (
    <AccordionItem
      ...
      className='border-border scroll-mt-24 overflow-hidden rounded-xl border bg-white px-5 transition-colors target:ring-2 target:ring-blue-400/60 target:ring-offset-2 hover:border-gray-300 data-[state=open]:border-gray-300 md:px-7 dark:bg-gray-950/40 dark:target:ring-blue-300/50 dark:target:ring-offset-gray-950 dark:hover:border-gray-700 dark:data-[state=open]:border-gray-700'
    >
      <AccordionTrigger className='cursor-pointer gap-4 py-5 hover:no-underline motion-safe:active:scale-[0.997] md:py-6 [&>svg]:size-5 [&>svg]:text-gray-400 dark:[&>svg]:text-gray-500'>
        <div className='flex flex-1 flex-col gap-1.5 text-left'>
          {periodText && (
            <p className='text-muted-foreground md:text-body-s text-xs tracking-tight tabular-nums'>
              {periodText}
            </p>
          )}
          <div className='flex items-baseline gap-4'>
            ...
            <span className='text-foreground text-lg leading-snug font-semibold text-balance break-keep md:text-xl'>
              {title}
            </span>
          </div>
        </div>
        ...
        <div className='border-t border-gray-100 pt-6 md:pt-7 dark:border-gray-800'>
          ...
          <ol className='flex flex-col divide-y divide-gray-100 dark:divide-gray-800'>
            {doneList?.map((done, j) => (
              ...
              >
                <h4 className='text-body-m text-foreground flex items-baseline gap-2.5 font-semibold text-pretty break-keep md:text-[17px]'>
                    ...
                    {done.details.map((detail, k) => {
                      if (typeof detail === 'string') {
                        return (
                          <li
                            key={k}
                            className='text-body-m list-outside list-disc leading-relaxed text-gray-800 marker:text-gray-300 dark:text-gray-200 dark:marker:text-gray-600'
                          >
...
const ProblemSolution: FC<ProblemSolutionProps> = ({
  ...
  <div className='border-border rounded-lg border bg-gray-50 px-4 py-3.5 md:px-5 md:py-4 dark:bg-gray-900/60'>
    <div className='flex flex-col gap-1.5 md:flex-row md:gap-5'>
      <span className='tracking-eyebrow shrink-0 text-[10.5px] font-semibold text-rose-500 uppercase md:mt-[5px] md:w-[68px] dark:text-rose-400'>
        {problemLabel}
      </span>
      <div className='text-body-m flex-1 leading-[1.75] text-gray-700 dark:text-gray-300'>
        <Markdown>{problem}</Markdown>
      </div>
    </div>
    <div className='my-3 h-px bg-gray-200/70 md:my-3.5 dark:bg-gray-700/60' />
    <div className='flex flex-col gap-1.5 md:flex-row md:gap-5'>
      <span className='tracking-eyebrow shrink-0 text-[10.5px] font-semibold text-blue-500 uppercase md:mt-[5px] md:w-[68px] dark:text-blue-400'>
        {solutionLabel}
      </span>
      <div className='text-body-m flex-1 leading-[1.75] text-gray-800 dark:text-gray-200'>
